...
ui <- dashboardPage(
  dashboardHeader(title = "Epistemas Media Kit"),
  
  dashboardSidebar(
    sidebarPanel(
      img(src = "epiSTEMas_Logo.png", height = 50, width = 50),
                  h1("seriously")
                )),
  dashboardBody(
    fluidRow(
    valueBoxOutput("spotify_box", width=2),
    valueBoxOutput("insta", width=2),
    valueBoxOutput("facebook", width=2),
    valueBoxOutput("twitter", width=2),
    valueBoxOutput("sitioweb", width=2),
    valueBoxOutput("boletin", width=2),

    valueBoxOutput("otraspod", width=2),
    valueBoxOutput("instareach", width=2),
    valueBoxOutput("facebookreach", width=2),
    valueBoxOutput("linkedin", width=2),
    plotOutput("edad")
#    fluidRow(
#      box(plotOutput("descargas_pais", height = 400)
#      ),
#      fluidRow(
#        box("Some text to try", br(), "what?!", 
#            plotOutput("edad"),
#            background = "black",
#            height = 600)))
  )
...
server <- function(input, output){
  ...
  output$spotify_box <- renderValueBox ({
    valueBox(
      value = redes[1, 2],
      subtitle = "Seguidores en Spotify",
      icon = icon("spotify"),
      color = "green")
  })
  
  output$otraspod <- renderValueBox ({
    valueBox(
      value = sum(26+2+19),
      subtitle = "Otras plataformas de podcasts",
      icon = icon("spotify"),
      color = "purple")
  })
  
  output$insta <- renderValueBox ({
    valueBox(
      value = redes[4, 2],
      subtitle = "Seguidores Instagram (engagement %)",
      icon = icon("instagram"),
      color = "fuchsia")
  })
  
  output$instareach <- renderValueBox ({
    valueBox(
      value = redes[6, 2],
      subtitle = "Alcance del mes anterior (cuentas de Instagram)",
      icon = icon("instagram"),
      color = "fuchsia")
  })

  output$sitioweb <- renderValueBox ({
    valueBox(
      value = redes[12, 2],
      subtitle = "Visitas/mes al sitio web",
      icon = icon("link"),
      color = "aqua")
  })
  
  output$boletin <- renderValueBox ({
    valueBox(
      value = redes[2, 2],
      subtitle = "Suscripciones al boletin (open rate %)",
      icon = icon("newspaper"),
      color = "yellow")
  })
  
  
  output$facebook <- renderValueBox ({
    valueBox(
      value = redes[7, 2],
      subtitle = "Seguidores en Facebook",
      icon = icon("facebook"),
      color = "blue")
  })
  
  output$facebookreach <- renderValueBox ({
    valueBox(
      value = redes[9, 2],
      subtitle = "Alcance del mes anterior (cuentas Facebook)",
      icon = icon("facebook"),
      color = "blue")
  })
  ...
  output$linkedin <- renderValueBox ({
    valueBox(
      value = redes[11, 2],
  ...
  output$descargas_pais <- renderPlot({ 
    descargaspais
  })
  
  output$descargas_mapa <- renderPlot({
    #text on hover
    streams_paises$hover <- with(streams_paises, paste(Pais, '<br>',
                                                       "Spotify", Spotify, "<br>",
                                                       "Otras plataformas", Ivoox))
